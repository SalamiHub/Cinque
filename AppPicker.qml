...
// App picker for the Edit view: same trigger/popup shape as qs.Ui's
// SearchableDropdown (which this is forked from, since that system
// component can't be edited in place and has no hook for the two things
// below), but with two deltas:
//   - recomputeFiltered() splits the query into whitespace-separated terms
//     and requires every term to substring-match label OR description,
//     mirroring Omarchy's own Super+Space launcher (MenuModel.js
//     matchesQuery: case-insensitive, exact substring, AND across terms,
//     not fuzzy) more closely than SearchableDropdown's single-substring
//     check.
//   - pinnedValue: the option with this value is excluded from filtering
//     and always kept first, for a "Custom command…" entry that isn't a
//     real app name and shouldn't disappear while typing.
//
// Empty query still shows the full option list unfiltered, so clicking
// through without typing works exactly like a plain dropdown.
Item {
  id: root
  ...
  function recomputeFiltered() {
    ...
    var rest = []
    for (var i = 0; i < options.length; i++) {
      if (root.pinnedValue !== "" && optionValue(options[i]) === root.pinnedValue) pinned.push(options[i])
      else rest.push(options[i])
    }
    var terms = searchField.text.toLowerCase().trim().split(/\s+/).filter(function(t) { return t.length > 0 })
    if (terms.length === 0) { filtered = pinned.concat(rest); return }
    var out = []
    for (var j = 0; j < rest.length; j++) {
      var lbl = optionLabel(rest[j]).toLowerCase()
      var desc = optionDescription(rest[j]).toLowerCase()
      var matchesAll = true
      for (var k = 0; k < terms.length; k++) {
        if (lbl.indexOf(terms[k]) === -1 && desc.indexOf(terms[k]) === -1) { matchesAll = false; break }
      }
      if (matchesAll) out.push(rest[j])
    }
    filtered = pinned.concat(out)
  }
